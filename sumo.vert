...
vec3 sphere(in vec3 p) {
  ...
  return vec3(
    cos(vertexId * .5 + time) * h,
    sin(vertexId * (.3 + volume /700000.) + time) * h,
    rz
  ) * .4;
}

vec3 rects(in vec3 p) {
  float xo = mod(floor(vertexId / 300.), 2.) - 1.;
  float yo = mod(floor(vertexId / 300. / 2.), 2.) - 1.;
  vec3 pRects = vec3(
  ...
  pRects.x += xo * 1.3;
  pRects.y += yo * 1.3;
  pRects.x += pRects.x*mod(vertexId,2.)*volume/500.;
  pRects.y += pRects.y*mod(vertexId,2.)*volume/500.;
...
void main() {
  ...
  float tt = time;
  float d = mod(tt, 9.) * .5;
  pos.x += pos.x*mod(vertexId,2.)*volume/300.;
  pos.y += pos.y*mod(vertexId,2.)*volume/300.;
